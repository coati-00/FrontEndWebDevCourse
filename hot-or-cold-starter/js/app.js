...
function newGame()
{
    ...
    $("form #guessButton").on('click', function(evt){
        ...
        var user_guess = $("form #userGuess").val();

        if (isValid(user_guess)) {
            userNumber($("form #userGuess").val(), secret_number);
            /* If guess is valid increment counter and add user guess to the bottom of the page */
...
function isValid(user_guess)
{

    /* If validation fails we do not want to add blank boxes for guess or increment the number of guesses, we want to notify the user to try again */
    ...
    //else we assume its true until we learn how to do stricter checks...
    else
    {
        return true;
    }
